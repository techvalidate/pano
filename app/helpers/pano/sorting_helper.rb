module Pano
  module SortingHelper
    ...
    def sort_link(name, param, search)
      if search.send("sorting_#{param}?")
        ...
        if search.send("sort_by_#{param}?")
          link_to name, sort_url_on(search, "#{param}_reverse"), class: 'sort-link selected desc', title: 'Click to toggle sort order'
        else
          link_to name, sort_url_on(search, param), class: 'sort-link selected asc', title: 'Click to toggle sort order'
        end
      else
        # show link to sort by this param
        link_to name, sort_url_on(search, param), class: 'sort-link', title: 'Click to sort'
      end
    end
