module Pano
  class DateFormBuilder < ActionView::Helpers::FormBuilder
    def formatted_date_field(name, options = {})
      ...
      # date = @template.text_field_tag("#{@object_name}[#{name}]", '', {is: 'date-input', value: @object.read_attribute(name) || @object.send(name), id: sanitize_to_id("#{@object_name}[#{name}]"), tabindex: 0, active: false}.update(options.stringify_keys))
